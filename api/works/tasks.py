...
def start_tests(report):
    ...
    file_request = urllib.request.urlopen(report.file.url).read()
    for test in tests:
        if test.test_type == Test.VALUE_EQUALS:
            status = test_value_function(file_request, test.function_name, test.get_value)
        else:
            status = test_type_function(file_request, test.function_name, test.get_value)
